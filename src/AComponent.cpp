...
{
	if (pin < 1 || pin > _NbPin)
		throw nts::PinLinkError("Invalid pin..", "nts::AComponent::setLink"); // Invalid Pin
	if (otherPin < 1)
		throw nts::PinLinkError("Invalid other pin..", "nts::AComponent::setLink");
	pin -= 1;
	if (_PinLink[pin])
		throw nts::PinLinkError("Pin already set..", "nts::AComponent::setLink");
